...
export default function CardPosterConfig() {
  ...
  const [usePosterMode, setUsePosterMode] = useState(true);
  ...
  return (
    <Card className="grow">
      ...
      <div className="mx-5 mt-5 mb-5 flex flex-col gap-x-5 gap-y-5">
        <p className="grow mr-5 text-3xl font-serif font-bold">Mode</p>
        <Switch
          isSelected={usePosterMode}
          onValueChange={setUsePosterMode}
          color="secondary"
        >
          {usePosterMode ? "Image Poster" : "Web Server (STA)"}
        </Switch>
        <p className="text-sm text-default-500">
          {usePosterMode
            ? "Post captured images to EzData regularly via WiFi"
            : "Run web server over WiFi (STA mode). On each boot, AP starts for 10s to allow reconfiguration, then connects to the configured WiFi"}
        </p>
      </div>
          ...
          <div className="mx-5 mt-5 mb-5 flex flex-col gap-x-5 gap-y-5">
            ...
            >
              {posterIntervalList.map((posterInterval) => (
                <SelectItem
                  key={posterInterval.value}
                  value={posterInterval.value}
                >
            ...
            {(onClose) => (
              <>
                ...
                <ModalBody>
                  <p>
                    {usePosterMode
                      ? "Start image poster with following config:"
                      : "Start web server (STA mode) with following config:"}
                  </p>
                  <p>
                    <Code color="default">Mode</Code>{" "}
                    {usePosterMode ? "Image Poster" : "Web Server (STA)"}
                  </p>
                  <p>
